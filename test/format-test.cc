...
::testing::AssertionResult check_write(const T &value, const char *type) {
  ...
  return ::testing::AssertionFailure() << "Value of: (Writer<" << type
                                       << ">() << value).str()\n"
                                       << "  Actual: " << actual << "\n"
                                       << "Expected: " << expected << "\n";
}
...
TEST(UtilTest, ParseNonnegativeInt) {
  ...
  EXPECT_THROW_MSG(parse_nonnegative_int(s, fmt::internal::error_handler()),
...
FMT_CONSTEXPR test_format_specs_handler check_specs(const char *s) {
  fmt::internal::specs_check_handler<test_format_specs_handler> checker(
...
struct test_format_string_handler {
  ...
  template <typename Iterator>
  FMT_CONSTEXPR Iterator on_format_specs(Iterator it) {
    return it;
  }
