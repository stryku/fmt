...
// Chack whether we can use unrestricted unions (e.g. unions with non-PODs
// members).
...
struct formatter {
  ...
  formatter_parse_result<typename ParseContext::iterator> parse(ParseContext &);
 ...
 public:
  class handle {
   public:
    ...
    void validate(basic_format_specs<typename Context::char_type> &specs) {
      return custom_.validate(custom_.value, specs);
    }
...
class basic_format_args {
 ...
 private:
  ...
  typename internal::type type(unsigned index) const {
    ...
    return static_cast<typename internal::type>((types_ & (mask << shift)) >>
                                                shift);
  }
 ...
 public:
  ...
  unsigned max_size() const {
    ...
    return static_cast<unsigned>(
        signed_types < 0 ? -signed_types
                         : static_cast<long long>(internal::max_packed_args));
  }
};
...
template <typename Context, typename S, typename... Args>
struct prepared_checked_args : format_arg_store<Context, Args...> {
  prepared_checked_args(const S &format_str, const Args &... args)
      : format_arg_store<Context, Args...>(args...) {
    internal::check_format_string<Args...>(format_str);
  }
